(function(){
 'use strict';
 window.DWGC2E_GLOSSARY_MERGE_DIALOG=(groups,active)=>new Promise(resolve=>{
  ...
  groups.forEach((row,i)=>{
   if(window.DWGC2E_GLOSSARY_MERGE.equal(row.local,row.remote))return;
  ...
  apply.onclick=()=>{if(!active())return finish(null);try{finish(window.DWGC2E_GLOSSARY_MERGE.resolve(groups,choices));}catch(e){error.textContent=e.message;}};
