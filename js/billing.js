(() => {
 ...
 async function init(){try{await member();const d=await api.billing.plans();say(d.message);for(const p of d.plans){const card=node('article',''),title=node('h2',p.name),desc=node('p',`Pro ${p.duration_days} 天 · 每月 100 万字符上限`),price=node('strong',money(p.price_cents)),b=node('button','立即购买','btn btn-primary');b.onclick=()=>checkout(p.id);card.append(title,desc,price,b);$('#plans').append(card);}await orders();if(pending)say('检测到未完成下单。选择原套餐可安全重试，或从最近订单查看付款状态。');}catch(e){$('#membership').textContent='会员信息未能完成同步，请刷新页面重试。';if($('#orderList').textContent==='正在加载订单…')$('#orderList').textContent='订单尚未加载，请刷新页面重试。';say(e.status===401?'请先登录与 APP 相同的账号，再购买会员套餐。':e.message,true);if(e.status===401)$('#loginLink').hidden=false;}}
