(() => {
 ...
 function schedule(o){stop();if(!stopped&&!document.hidden&&['pending','expired'].includes(o.status))timer=setTimeout(refresh,Math.min(30000,3000*2**failures));}
 function show(o){clearTimeout(expiryTimer);current=o;if(o.status==='pending'&&Date.parse(o.expiresAt)>Date.now())expiryTimer=setTimeout(()=>{if(current?.orderNo===o.orderNo){renderQr(o);$('#scanHint').hidden=true;$('#countdown').textContent='二维码展示期限已结束；系统继续确认付款，请勿再扫旧码。';}},Math.min(2147483647,Math.max(0,Date.parse(o.expiresAt)-Date.now()+20))); $('#checkout').hidden=false;$('#checkoutTitle').textContent=o.planName;$('#paymentAmount').textContent=money(o.payableCents);$('#paymentChannel').textContent=o.channel==='wxpay'?'支付方式：微信':'支付方式：支付宝';$('#orderNo').textContent=o.orderNo;$('#paymentStatus').textContent=statusText(o);
 const left=Math.max(0,Math.ceil((Date.parse(o.expiresAt)-Date.now())/1000));$('#countdown').textContent=o.status==='paid'?'':left?'二维码展示剩余 '+Math.floor(left/60)+' 分 '+left%60+' 秒':'二维码展示期限已结束；系统继续确认付款，请勿再扫旧码。';
 $('#scanHint').hidden=!(o.allowedActions?.pay&&left>0);renderQr(o);schedule(o);
 }
 async function afterPaid(o){if(o.status!=='paid')return;const selected=selection;const active=()=>!stopped&&selected===selection&&current?.orderNo===o.orderNo;try{await clearPaid(o);await member();if(active())say('支付成功，会员与额度已同步。');}catch{if(active())say('支付成功，会员信息同步失败。请刷新账户，勿再次付款。',true);}}
 ...
 if(pending&&(pending.planId!==planId||pending.channel!==channel)){say('存在未确认购买，请先查看原订单；不会重新创建另一笔付款。',true);if(pending.orderNo){++selection;show(await api.billing.status(pending.orderNo));}return;}
 if(!pending){pending={planId,channel,key:crypto.randomUUID()};save();}
 const o=await api.billing.checkout({planId:pending.planId,channel:pending.channel},pending.key);pending.orderNo=o.orderNo;save();++selection;show(o);
 ...
 await member();await orders();if(pending?.orderNo){++selection;show(await api.billing.status(pending.orderNo));await afterPaid(current);}else if(pending){say('检测到未完成购买，重试将复用原标识。');const retry=node('button','确认原购买','btn btn-primary');retry.onclick=()=>checkout(pending.planId);$('#plans').append(retry);}
 ...
 $('#refreshOrder').onclick=()=>refresh(true);$('#moreOrders').onclick=()=>orders(true).catch(e=>say(e.message,true));document.addEventListener('visibilitychange',()=>{if(document.hidden)stop();else refresh();});window.addEventListener('online',()=>refresh());window.addEventListener('storage',e=>{if(e.key===savedKey){try{pending=load();refresh();}catch(err){say(err.message,true);}}});window.addEventListener('pagehide',()=>{stopped=true;stop();});window.addEventListener('pageshow',()=>{stopped=false;if(current)refresh();});init();
})();
