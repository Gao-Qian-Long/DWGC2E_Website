(() => {
 'use strict';const base=(window.DWGC2E_SITE||{}).apiBaseUrl||'/api';
 ...
 window.DWGC2E_ADMIN_AUTH={check:()=>session(),login:(key,username)=>session('POST',key,username),logout:async()=>{await session('DELETE');channel?.postMessage('logout');window.dispatchEvent(new Event('admin-logout'));}};
