(() => {
  ...
  let historyFilterTimer = 0, historyFilterComposing = false;
  const historyFilterSchedule = () => {
    if (historyFilterComposing) return;
    clearTimeout(historyFilterTimer);
    historyFilterTimer = setTimeout(render, 180);
  };
  filter.addEventListener('compositionstart', () => { historyFilterComposing = true; });
