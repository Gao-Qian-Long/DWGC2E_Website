(() => {
  ...
  let rows = [], nextCursor=null, loading=false;const more=document.createElement('button');more.className='btn btn-ghost';more.textContent='加载更多';more.hidden=true;msg.after(more);
  ...
  async function load(append=false){if(loading)return;loading=true;more.disabled=true;try{const data=await window.DWGC2E_API.history.list(append?nextCursor:null);rows=append?[...rows,...normalize(data)]:normalize(data);nextCursor=data.nextCursor;more.hidden=!nextCursor;render();say(rows.length?'':'暂无翻译记录。');}catch(error){render();say(error.message||'翻译记录暂时无法读取。',true);}finally{loading=false;more.disabled=false;}}
