(() => {
  ...
  const safeDownload=v=>{try{const u=new URL(String(v),location.href);return u.protocol==='https:'?u.href:''}catch{return ''}};
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const render=()=>{const q=filter.value.toLowerCase(),data=rows.filter(x=>[x.file_name,x.status,x.created_at].join(' ').toLowerCase().includes(q));list.innerHTML=data.length?data.map(x=>`<tr><td>${esc(x.file_name||'未命名任务')}</td><td>${esc(x.status||'处理中')}</td><td>${esc(x.source_language||'自动')}</td><td>${esc(x.target_language||'—')}</td><td>${esc(x.created_at||'—')}</td><td>${x.id||x.task_id?`<button class="btn btn-ghost btn-sm" data-detail="${esc(x.id||x.task_id)}">刷新</button>`:''}${safeDownload(x.download_url)?`<a class="btn btn-primary btn-sm" href="${esc(safeDownload(x.download_url))}" target="_blank" rel="noopener noreferrer">下载</a>`:'—'}</td></tr>`).join(''):'<tr><td colspan="6" class="table-empty">暂无翻译记录。</td></tr>'};
