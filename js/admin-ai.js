...
(() => {
  ...
  $('userLogin').addEventListener('submit', async event => { event.preventDefault(); if (loading) return; const key = $('userKey').value; const username = $('userName').value.trim(); if (key.trim() !== key || key.length < 32) { setMessage('userMessage', '请输入已部署的管理员密钥（至少 32 个字符，不能含首尾空格）。', true); return; } setBusy(true); setMessage('userMessage', '正在验证后台登录…'); try { await auth.login(key, username); $('userKey').value = ''; showWorkspace(); await loadAll(); } catch (error) { $('userKey').value = ''; setMessage('userMessage', describe(error), true); } finally { setBusy(false); } });
