(() => {
 ...
 async function load(){if(busy)return;busy=true;$('#plansReload').disabled=true;$('#plansMessage').textContent='正在同步套餐权益…';
  ...
  try{const requests=[signedIn?api.billing.plans():api.billing.publicPlans()];if(signedIn)requests.push(api.billing.entitlements());
   const [catalog,ent]=await Promise.all(requests);
   if(signedIn){if(!auth.active())return;const tier=ent.subscription.plan_name;
   ...
   }else{$('#currentTier').textContent='当前展示所有套餐的价格与权益';$('#currentUsage').textContent='';$('#currentAddons').replaceChildren();}
   const tier=ent?.subscription?.plan_name;
   $('#benefitCards').replaceChildren();for(const p of catalog.plans){const current=!!tier&&p.id===tier,go=p.id==='go',card=node('article','','benefit-card'+(current?' is-current':''));card.dataset.plan=p.id;if(p.description)card.append(node('p',p.description,'field-hint'));
   ...
   }$('#plansMessage').textContent=signedIn?'权益已同步。本页不会创建订单或发起付款。':'价格与权益已同步，无需登录。购买请前往订单与付款页。';if(signedIn)auth.ready();
  }catch(error){auth.failure(error);if(auth.active()){auth.error();$('#plansMessage').textContent=error.message||'读取失败，请刷新重试。';}else{$('#plansMessage').textContent=error.message||'套餐信息读取失败，请刷新重试。';}}
