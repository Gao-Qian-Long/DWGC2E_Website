(() => {
 ...
 async function refresh(){try{
  ...
  const downloadAction=document.querySelector('#download .download-action');
  const host=downloadAction||document.querySelector('.client-copy')||document.querySelector('#releaseList');if(host){document.getElementById('managedRelease')?.remove();const box=document.createElement('div');box.id='managedRelease';box.className='managed-release'+(downloadAction?' managed-release--download':'');if(release.backup_download_url&&safe(release.backup_download_url)){const a=document.createElement('a');a.className='managed-release-link';a.href=release.backup_download_url;a.target='_blank';a.rel='noopener noreferrer';a.setAttribute('aria-label','使用备用地址下载 Windows 客户端（在新窗口打开）');a.append(text('span','备用下载'),text('span','↗'));box.append(a);}if(release.release_notes){const d=document.createElement('details');d.append(text('summary','V'+release.latest_version+' 更新说明'),text('p',release.release_notes));box.append(d);}if(box.childElementCount)host.append(box);}
