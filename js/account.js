(() => {
  ...
  const loadDashboard = async token => {
    ...
    $("#profileName").textContent = profile.display_name || profile.account || profile.email || 'DWGC2E 用户'; const identity = profile.account && profile.email && profile.account !== profile.email ? `账号：${profile.account} · ${profile.email}` : (profile.account || profile.email || ''); $("#profileEmail").textContent = identity;
  ...
  const sendVerificationCode = async (purpose, email, button) => {
    ...
    try {
      await request(purpose === 'register' ? '/v1/auth/register/request-code' : '/v1/auth/password/request-code', { method: 'POST', body: JSON.stringify({ email }) });
      setMessage('验证码已发送，请检查邮箱（10 分钟内有效）。');
      let seconds = 60; button.textContent = `${seconds}s 后重试`;
      codeTimer = setInterval(() => { seconds -= 1; button.textContent = seconds ? `${seconds}s 后重试` : '发送验证码'; if (!seconds) { clearInterval(codeTimer); button.disabled = false; } }, 1000);
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      ...
      if (mode === 'register') { if (!data.email || !data.register_code) return setMessage('请填写邮箱和邮箱验证码。', true); await request('/v1/auth/register', { method: 'POST', body: JSON.stringify({ ...data, verification_code: data.register_code }) }); }
  ...
  setMode('login');
  try { const saved = JSON.parse(sessionStorage.getItem(storageKey) || 'null'); if (saved?.token && (!saved.expiresAt || new Date(saved.expiresAt) > new Date())) { loadDashboard(saved.token).catch(() => { sessionStorage.removeItem(storageKey); $('#accountPanel').hidden = true; $('#authPanel').hidden = false; document.documentElement.classList.remove('has-session'); document.body.classList.remove('has-session'); $('#accountLoading')?.remove(); $('#accountPanel').hidden = true; $('#authPanel').hidden = false; setMode('login'); }); } else { sessionStorage.removeItem(storageKey); $('#accountLoading')?.remove(); document.documentElement.classList.remove('has-session'); document.body.classList.remove('has-session'); } } catch { sessionStorage.removeItem(storageKey); document.body.classList.remove('auth-loading'); }
})();
