(() => {
  ...
  setMode('login');
  try { const saved = JSON.parse(sessionStorage.getItem(storageKey) || 'null'); if (saved?.token && (!saved.expiresAt || new Date(saved.expiresAt) > new Date())) { loadDashboard(saved.token).catch(() => { sessionStorage.removeItem(storageKey); $('#accountPanel').hidden = true; $('#authPanel').hidden = false; document.body.classList.remove('has-session'); $('#accountLoading')?.remove(); $('#accountPanel').hidden = true; $('#authPanel').hidden = false; setMode('login'); }); } else { sessionStorage.removeItem(storageKey); $('#accountLoading')?.remove(); document.body.classList.remove('has-session'); } } catch { sessionStorage.removeItem(storageKey); document.body.classList.remove('auth-loading'); }
})();
